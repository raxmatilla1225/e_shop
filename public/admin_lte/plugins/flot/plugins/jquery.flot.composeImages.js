...
function, which generates temporary users out of them. This function
...
failure in generating the temporary image. Temporary users are required
...
After all temporary users are generated, they are overlapped using
...
temporary users load their data.
*/
    ...
    function getGenerateTempImg(tempImg, canvasOrSvgSource) {
        ...
        return function doGenerateTempImg(successCallbackFunc, failureCallbackFunc) {
            ...
            tempImg.onabort = function(evt) {
                ...
                successCallbackFunc(tempImg); //call successCallback, to allow snapshot of all working users
            };
    ...
    function prepareImagesToBeComposed(sources, destination) {
        ...
        if (sources.length === 0) {
            ...
            if ((maxX - minX <= 0) || (maxY - minY <= 0)) {
                result = NEGATIVEIMAGESIZE; //this might occur on hidden users
            } else {
